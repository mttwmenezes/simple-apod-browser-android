...
import br.com.mathsemilio.simpleapodbrowser.ui.common.permission.PermissionRequestResult.DENIED
import br.com.mathsemilio.simpleapodbrowser.ui.common.permission.PermissionRequestResult.DENIED_PERMANENTLY
import br.com.mathsemilio.simpleapodbrowser.ui.common.permission.PermissionRequestResult.GRANTED

class PermissionHandler(
    private val activity: AppCompatActivity
) : BaseObservable<PermissionHandler.RequestResultListener>() {
    ...
    fun onRequestPermissionResult(
        ...
        if (androidPermissions.isEmpty() || grantResults.isEmpty())
            notify { it.onPermissionRequestResult(DENIED) }

        if (validRequestCode(requestCode)) {
            when {
                permissionGranted(grantResults) -> {
                    notify { it.onPermissionRequestResult(GRANTED) }
                }
                ...
                else -> notify { it.onPermissionRequestResult(DENIED_PERMANENTLY) }
            }
        }
    }
